...
function registryDataForSymbol(symbol: string): RegistryData {
    const registryData = TOKEN_REGISTRY_TRACKED_TOKENS.find(tokenObject => tokenObject.symbol === symbol);
...
export class TokenAmount {
    ...
    constructor(amount: number, symbol: string) {
        ...
        this.rawAmount = TokenAmount.convertToRaw(new BigNumber(amount.toString()), this.data.numDecimals);
    }
